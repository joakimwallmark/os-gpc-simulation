...
cl <- makeCluster(min(detectCores(logical = F), 4)) # Too many cores needs too much RAM memory
registerDoParallel(cl)
res <- foreach(sn = 1:length(scenarios), .packages = c("mirt", "TestGardener")) %dopar% {
  scen <- scenarios[[sn]]
  ...
  if (length(scen$mirt_items)>0) {
    for (item in 1:length(scen$mirt_items)) {
  ...
  if (length(scen$os_items)>0) {
    for (item in (length(scen$mirt_items)+1):no_items) {
      WListi <- os_mod$parList[[os_cycles]]$WfdList[[scen$os_items[item-length(scen$mirt_items)]]]
      surpisal <- eval.surp(scen$test_taker_thetas[, "tg"], WListi$Wfd)
      true_probs[[item]] <- WListi$M^(-surpisal)
    }
  }
  
  item_fit <- list(probs_mirt = list(), probs_tg = list(), 
  ...
  for (r in 1:iterations) {
    ...
    for (item in 1:no_items) {
      ...
      # os model probs
      WListi <- sim_os_mod$parList[[os_cycles]]$WfdList[[item]]
      surpisal <- eval.surp(sim_os_thetas, WListi$Wfd)
      sim_tg_probs <- WListi$M^(-surpisal)
      true_p <- true_probs[[item]]
  ...
  all_items_measures <- get_simulation_measures(item_fit, 
                                                true_probs, 
                                                1:length(true_probs), 
                                                item_ind, 
                                                response_cats)
  ...
  if (length(scen$mirt_items)>0) {
    mirt_items_measures <- get_simulation_measures(item_fit, 
                                                   true_probs, 1:length(scen$mirt_items), 
                                                   item_ind, 
                                                   response_cats)
  }
  if (length(scen$os_items)>0) {
    item_start_ind <- length(scen$mirt_items)+1
    os_items_measures <- get_simulation_measures(item_fit, 
                                                 true_probs, 
