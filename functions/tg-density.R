...
tg_theta_density <- function(tg_thetas, alpha = 1.4) {
  n <- length(tg_thetas)
  dens <- ssden(~tg_thetas, alpha = alpha, domain = data.frame(tg_thetas = c(0, 100)))
  p_max <- sum(tg_thetas == 100) / n
  p_min <- sum(tg_thetas == 0) / n

  df <- data.frame(x = seq(0, 100, by = 0.1), y = (1 - p_max - p_min) * sapply(seq(0, 100, by = 0.1), function(x) {
    dssden(dens, x)
  }))
