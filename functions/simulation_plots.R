sim_fit_plot_bias <- function(x, measures, cat, x_lab = "Arc length", y_lab = "Bias", title = "") {
  ...
  if (title != "") return(plot + ggtitle(title))
  else return(plot)
}
...
plots_mse_kl_cdf_perc <- function(scen_name, x, mirt_rmse, tg_rmse, mirt_kl, tg_kl,
                                  file_name_mse, file_name_kl) {
  df_mse <- data.frame(x = x, y = c(rowMeans(mirt_rmse), rowMeans(tg_rmse)), Model <- c(rep("GPC", length(x)), rep("Optimal score", length(x))))
  mse_plot <- ggplot(df_mse, aes(x=x, y=y, linetype=Model)) +
    ...
    scale_y_continuous(limits = c(NA, NA),
                       expand = expansion(mult = c(0.05, 0.05), add = c(0, 0)))
  df_mse_mirt_items <- data.frame(x = x, y = c(rowMeans(mirt_rmse[, 1:length(scen$mirt_items)]), rowMeans(tg_rmse[, 1:length(scen$mirt_items)])), Model <- c(rep("GPC", length(x)), rep("Optimal score", length(x))))
  mse_plot_mirt <- ggplot(df_mse_mirt_items, aes(x=x, y=y, linetype=Model)) +
    ...
    scale_y_continuous(limits = c(NA, NA),
                       expand = expansion(mult = c(0.05, 0.05), add = c(0, 0)))
  df_mse_tg_items <- data.frame(x = x, y = c(rowMeans(mirt_rmse[, -(1:length(scen$mirt_items))]), rowMeans(tg_rmse[, -(1:length(scen$mirt_items))])), Model <- c(rep("GPC", length(x)), rep("Optimal score", length(x))))
  mse_plot_tg <- ggplot(df_mse_tg_items, aes(x=x, y=y, linetype=Model)) +
    ...
    scale_y_continuous(limits = c(NA, NA),
                       expand = expansion(mult = c(0.05, 0.05), add = c(0, 0)))
  df_kl <- data.frame(x = x, y = c(rowMeans(mirt_kl), rowMeans(tg_kl)), Model <- c(rep("GPC", length(x)), rep("Optimal score", length(x))))
  ...
  df_kl_mirt_items <- data.frame(x = x, y = c(rowMeans(mirt_kl[, 1:length(scen$mirt_items)]), rowMeans(tg_kl[, 1:length(scen$mirt_items)])), Model <- c(rep("GPC", length(x)), rep("Optimal score", length(x))))
  ...
  df_kl_tg_items <- data.frame(x = x, y = c(rowMeans(mirt_kl[, -(1:length(scen$mirt_items))]), rowMeans(tg_kl[, -(1:length(scen$mirt_items))])), Model <- c(rep("GPC", length(x)), rep("Optimal score", length(x))))
  kl_plot_tg <- ggplot(df_kl_tg_items, aes(x=x, y=y, linetype=Model)) +
    ...
    scale_y_continuous(limits = c(NA, NA),
                       expand = expansion(mult = c(0.05, 0.05), add = c(0, 0)))
  ggsave(file_name_mse, plot = mse_plot/mse_plot_mirt/mse_plot_tg + plot_annotation(tag_levels = "A"), width = 21-2*2.54, height = 20, units = "cm")
  ggsave(file_name_kl, plot = kl_plot/kl_plot_mirt/kl_plot_tg + plot_annotation(tag_levels = "A"), width = 21-2*2.54, height = 20, units = "cm")
