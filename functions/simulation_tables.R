...
sim_item_table <- function(measure_list) {
  item_table <- data.frame(
    "",
    seq_along(ncol(measure_list$avg_mirt_kl)),
    sapply(measure_list$avg_mirt_bias, function(x) {
      mean(abs(x))
    }),
    sapply(measure_list$avg_os_bias, function(x) {
      mean(abs(x))
    }),
    sapply(measure_list$avg_mirt_see, function(x) {
      mean(x)
    }),
    sapply(measure_list$avg_os_see, function(x) {
      mean(x)
    }),
    sapply(measure_list$avg_mirt_rmse, function(x) {
      mean(x)
    }),
    sapply(measure_list$avg_os_rmse, function(x) {
      mean(x)
    }),
    colMeans(measure_list$avg_mirt_kl),
    colMeans(measure_list$avg_os_kl)
  )
  colnames(item_table) <- c(
    "True item type", "item",
    "mirt bias", "os bias",
    "mirt SE", "os SE",
    "mirt RMSE", "os RMSE",
