...
test_takers[[3]] <- sort(rnorm(5000))
test_takers[[4]] <- sort(rsn(5000, xi, omega, alpha))
# test_takers[[5]] <- sort(rnorm(10000))
# test_takers[[6]] <- sort(rsn(10000, xi, omega, alpha))
...
# SETUP SCENARIOS ---------------------------------------------------------
items <- 1:length(response_cats)
resp_cats <- sort(as.numeric(unique(names(table(response_cats)))), decreasing = T)
...
for(length in test_lengths) {
  for(prop_os_items in props_os_items) {
    ...
    if (prop_os_items == 1) {
      ...
      item_sample <- sample(response_cats, length)
      os_sample <- sample(1:length(item_sample), prop_os_items*length)
    ...
    for(sample in 1:length(test_takers)) {
      ...
      if (is.null(mirt_items)) {
        ...
        true_ability_list$surp_al <- arc_length_surp(os_mod, sample_tg, total_cat, max_surp, item_ids = os_items, mesh_accuracy = 10001)[, 1]
      } else if (is.null(os_items)) {
        total_cat <- sum(response_cats[c(mirt_items)])
        true_ability_list$surp_al <- arc_length_surp(mirt_mod, mirt_samp, total_cat, max_surp, item_ids = mirt_items, mesh_accuracy = 10001)[, 1]
      } else {
        true_ability_list$surp_al <- arc_length_surp_mixed(mirt_mod, mirt_samp, mirt_items, os_mod, sample_tg, os_items, total_cat, 100, mesh_accuracy = 101)[, 1]
      }
      
      surp_rand_mirt <- get_dist_travelled_mirt(mirt_mod, mirt_items, sort(c(mirt_samp, seq(min(mirt_samp), max(mirt_samp), length.out = 10000))))
      surp_rand_tg <- get_dist_travelled_tg(os_mod, os_items, sort(c(sample_tg, seq(0, 100, length.out = 10000))))
      
      prob_rand_mirt <- get_dist_travelled_mirt(mirt_mod, mirt_items, sort(c(mirt_samp, seq(min(mirt_samp), max(mirt_samp), length.out = 10000))), mode = "prob")
      prob_rand_tg <- get_dist_travelled_tg(os_mod, os_items, sort(c(sample_tg, seq(0, 100, length.out = 10000))), mode = "prob")
      
      entr_rand_mirt <- get_dist_travelled_mirt(mirt_mod, mirt_items, sort(c(mirt_samp, seq(min(mirt_samp), max(mirt_samp), length.out = 10000))), mode = "entropy")
      entr_rand_tg <- get_dist_travelled_tg(os_mod, os_items, sort(c(sample_tg, seq(0, 100, length.out = 10000))), mode = "entropy")
      # pick out the thetas for the sample. Add same person thetas from both models
      true_ability_list$surp_dist <- surp_rand_tg[match(sample_tg, surp_rand_tg[, 1]), 2] + surp_rand_mirt[match(mirt_samp, surp_rand_mirt[, 1]), 2]
      true_ability_list$prob_dist <- prob_rand_tg[match(sample_tg, prob_rand_tg[, 1]), 2] + prob_rand_mirt[match(mirt_samp, prob_rand_mirt[, 1]), 2]
      true_ability_list$entr_dist <- entr_rand_tg[match(sample_tg, entr_rand_tg[, 1]), 2] + entr_rand_mirt[match(mirt_samp, entr_rand_mirt[, 1]), 2]
      
      
      scnario_name <- paste(data_string, "-n", n, "-i", length, "-os-prop", round(prop_os_items, 2), "-", test_takers_pop[sample], sep = "")
      scenarios[[scenario_ind]] <- list(name = scnario_name,
