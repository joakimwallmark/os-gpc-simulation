...
for (sn in 1:length(scenarios)) {
  ...
  if (is.null(mirt_items_measures) | is.null(os_items_measures)) { # if we only have 1 item type
    ...
    # p1_kldi/p2/p3 + plot_layout(guides = "collect") & theme(legend.position = "bottom") 
    ...
    # p1_bias/p2/p3 + plot_layout(guides = "collect") & theme(legend.position = "bottom")
    ...
    # p1_rmse/p2/p3 + plot_layout(guides = "collect") & theme(legend.position = "bottom") 
...
plot_item <- 17
plot_item <- 24
item_plot <- sim_item_fit_plot_bias(x, all_items_measures, 2, item = plot_item, "% arc length", "Probability bias", "GPC items")
item_plot
item_plot2 <- sim_item_fit_plot_bias_col(x, all_items_measures, item = plot_item, "% arc length", "Probability bias", "GPC items")
item_plot2
...
mirt_bias <- rowMeans(sapply(all_items_measures$avg_mirt_bias, function(x) {rowMeans(abs(x))} ))
os_bias <- rowMeans(sapply(all_items_measures$avg_os_bias, function(x) {rowMeans(abs(x))} ))

mirt_se <- rowMeans(sapply(all_items_measures$avg_mirt_see, function(x) {rowMeans(x)} ))
os_se <- rowMeans(sapply(all_items_measures$avg_os_see, function(x) {rowMeans(x)} ))

mirt_rmse <- rowMeans(sapply(all_items_measures$avg_mirt_rmse, function(x) {rowMeans(x)} ))
os_rmse <- rowMeans(sapply(all_items_measures$avg_os_rmse, function(x) {rowMeans(x)} ))
